...
async function saveCard(client: PoolClient, card: ScryfallCard) {
  ...
  if (owned.rows[0]) {
    ...
    await client.query(
      `UPDATE cards SET image_url=coalesce(image_url,$2),oracle_text=coalesce($3,oracle_text),price_usd_cents=coalesce($4,price_usd_cents),mana_cost=coalesce($5,mana_cost),mana_value=coalesce($6,mana_value),type_line=coalesce($7,type_line),colors=$8,color_identity=$9,price_checked_at=now(),commander_legal=true,updated_at=now() WHERE id=$1`,
      [owned.rows[0].id,imageUrl(card),card.oracle_text??null,ownedPrice,card.mana_cost??null,card.cmc??null,card.type_line??null,card.colors??[],card.color_identity],
    );
  ...
  const saved = await client.query<{ id: string }>(
    `INSERT INTO cards (external_id,name,set_code,set_name,collector_number,mana_cost,mana_value,type_line,oracle_text,colors,color_identity,rarity,image_url,commander_legal,price_usd_cents,price_checked_at)
     ...
     RETURNING id`,
    [`scryfall:${card.id}`,card.name,card.set.toUpperCase(),card.set_name,card.collector_number??null,card.mana_cost??null,card.cmc??null,card.type_line??"",card.oracle_text??null,card.colors??[],card.color_identity,card.rarity??null,imageUrl(card),price],
  );
...
export async function createCommanderDeck(input: { name: string; commanderName: string; commanderId:string; budgetCents: number | null; goals:string|null }) {
  ...
  const identity = commander.color_identity.length ? commander.color_identity.join("") : "C";
  ...
  try {
    const plan=await askDeckAssistant({commander,candidates:spellCandidates,ownedNames,budgetCents:input.budgetCents,goals:input.goals});
  ...
  try {
    ...
    const description=assistantUsed?`AI-assisted build around ${commander.name}. ${assistantSummary}`:`Generated around ${commander.name} using the local Scryfall fallback. Add OPENAI_API_KEY to enable assistant-built lists.`;
