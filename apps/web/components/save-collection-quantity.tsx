...
export function SaveCollectionQuantity({ itemId, quantity }: { itemId: string; quantity: number }) {
  ...
  return <form action={formAction} onSubmit={() => setDirty(false)} className="collection-quantity-form">
    <input type="hidden" name="itemId" value={itemId} />
    <label>Qty <input name="quantity" type="number" min="0" max="999" defaultValue={quantity} disabled={pending} onChange={() => setDirty(true)} /></label>
