...
type Printing={id:string;name:string;set:string;setName:string;manaCost?:string;type?:string;imageUrl:string|null};
...
export function LibraryPrintingSearch() {
  ...
  return <div className="printing-autocomplete"><div className="searchbar compact" role="combobox" aria-expanded={results.length>0} aria-controls="printing-results"><Search size={17}/><input value={query} onChange={event=>{setQuery(event.target.value);if(event.target.value.trim().length<2){setResults([]);setError("");}}} placeholder="e.g. Lightning Bolt" autoComplete="off" aria-label="Find a card printing"/>{query&&<InputClearButton label="Clear card printing search" onClear={()=>{setQuery("");setResults([]);setError("");}}/>}{loading?<LoaderCircle className={`combobox-loader${query?" has-clear":""}`} size={15}/>:null}</div>{error&&<p className="notice">{error}</p>}{results.length>0&&<div className="printing-results" id="printing-results">{results.map(card=><form action={addCardToLibrary} className="printing-result" key={card.id}><input type="hidden" name="externalId" value={card.id}/><HoverCardPreview src={card.imageUrl} name={card.name}><span className="printing-result-copy"><strong>{card.name}</strong><small>{card.setName} · {card.set}</small><span><ManaCost cost={card.manaCost}/> {card.type}</span></span></HoverCardPreview><label>Qty<input aria-label={`Quantity of ${card.name}`} name="quantity" type="number" min="1" defaultValue="1"/></label><button aria-label={`Add ${card.name}`}><Plus size={14}/></button></form>)}</div>}</div>;
}
