...
export default async function DecksPage() {
  const decks = (
    ...
    }>(`SELECT d.id,d.name,d.format,d.description,coalesce(sum(dc.quantity),0)::int cards,coalesce(sum(CASE WHEN c.type_line LIKE '%Basic%' AND c.type_line LIKE '%Land%' THEN dc.quantity ELSE least(dc.quantity,coalesce(o.qty,0)) END),0)::int owned,coalesce(sum(CASE WHEN c.type_line LIKE '%Basic%' AND c.type_line LIKE '%Land%' THEN 0 ELSE greatest(dc.quantity-coalesce(o.qty,0),0)*coalesce(c.price_usd_cents,0) END),0)::int missing_cost,
