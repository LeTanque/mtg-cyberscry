...
import { updateCollectionQuantity } from "@/app/actions";
...
export default async function LibraryPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  return <div className="shell page">
    ...
    <div className="split-layout"><section>
      ...
      <div className="collection-grid">{library.map(card => <article className="collection-card" key={card.item_id}><CardArt src={card.image_url} name={card.name}/><div className="collection-info"><span className="set">{card.set_code} · {card.condition.replace("_"," ")}</span><h3>{card.name}</h3><p><ManaCost cost={card.mana_cost}/> {card.type_line}</p><div className="card-bottom"><strong>{money(card.price_usd_cents)}</strong><form action={updateCollectionQuantity}><input type="hidden" name="itemId" value={card.item_id}/><label>Qty <input name="quantity" type="number" min="0" defaultValue={card.quantity}/></label><button>Save</button></form></div></div></article>)}</div>
